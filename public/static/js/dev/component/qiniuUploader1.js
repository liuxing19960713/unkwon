define(function (){
    return function (elementImg, elementInput, isMulti){
        ...
        var uploader = Qiniu.uploader({
            ...
            browse_button: 'pickfile1',         // 上传选择的点选按钮，**必需**
            ...
            container: 'container1',             // 上传区域 DOM ID，默认是 browser_button 的父元素，
            max_file_size: '1000mb',             // 最大文件体积限制
            flash_swf_url: appConfig.publicPath + '/static/component/plupload/js/Moxie.swf',  //引入 flash,相对路径
            max_retries: 3,                     // 上传失败最大重试次数
            filters: {
                mime_types: [
                    //只允许上传视频文件 （注意，extensions中，逗号后面不要加空格）
                    { title: "视频文件", extensions: "rm,avc,rmvb,wmv,avi,mp4" }
                ]
            },
            ...
            init: {
                ...
                'UploadProgress': function(up, file) {
                    // 每个文件上传时,处理相关的事情
                     ...
                     $('#jindutiao').html(file.percent + '%');
                     $('#jindutiao').css("width",file.percent + '%');
                ...
                'FileUploaded': function(up, file, info) {
                    ...
                    var sourceLink = domain + res.key; //获取上传成功后的文件的Url
                     ...
                     $('#videoshow').attr("src", sourceLink);
                    //.attr("src", sourceLink);
                    // elementInput.val(sourceLink);
                    $('.text_video').html("上传成功，你可以提交了");
                    $('#show').css("display",'none');
