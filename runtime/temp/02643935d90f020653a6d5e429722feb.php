<?php if (!defined('THINK_PATH')) exit(); /*a:1:{s:83:"/var/www/html/Unkonwn/public/../application/admin/view/appannouncements/editor.html";i:1541563427;}*/ ?>
<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
...
<body>
    <div id="KinBox">
        ...
        <div class="titleBox">
            ...
            <input id="title" type="text" value="<?php echo $data[0]['title']; ?>">
            <span style="margin-left:20px;">公告类型：</span>
            <select name="sub_type" id="sub_type">
                <option value="user"<?php if($data[0]['sub_type'] == 'user'): ?>selected<?php endif; ?>>用户端推送</option>
                <option value="doctor"<?php if($data[0]['sub_type'] == 'doctor'): ?>selected<?php endif; ?>>医生端推送</option>
            </select>
            <span style="margin-left:20px;">是否隐藏：</span>
            <select name="show" id="show">
                <option value="show"<?php if($data[0]['show'] == 'show'): ?>selected<?php endif; ?>>显示</option>
                <option value="hide"<?php if($data[0]['show'] == 'hide'): ?>selected<?php endif; ?>>隐藏</option>
            </select>
        </div>
        <!-- 加载编辑器的容器 -->
